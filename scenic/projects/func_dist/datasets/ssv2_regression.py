...
import json
import os
from typing import Any, Dict, Iterator, Iterable, Optional, Text, Tuple, Union
...
class Ssv2DatasetFactory(video_tfrecord_dataset.TFRecordDatasetFactory):

  def _add_task_label(self):
  ...
  def _add_video_id(self):
    ...
    self.sampler_builder.add_fn(
        ...
        fn_name='video_id_sparse_to_dense')
    self.sampler_builder.add_fn(
        fn=lambda x: processors.set_shape(x, (1,)),
        feature_name='video_id',
        fn_name='video_id_set_shape')
    
    self.sampler_builder.add_fn(
        fn=data_path_to_video_id,
        feature_name='video_id',
        fn_name='video_id_from_data_path')

  def _add_frame_rate(self):
    self.parser_builder.parse_feature(
        feature_name='image/frame_rate',
        feature_type=tf.io.FixedLenFeature((1,), dtype=tf.float32),
        output_name='frame_rate',
        is_context=True)


class RegressionDatasetFactory(Ssv2DatasetFactory):
  ...
  def _build(self,
    ...
    Args:
      ...
      frames_per_video: the number of frames to sample from a video for each 
          example.
      augment_goals: whether to use any future frame or only the final frame as
        the goal.
    """
    self._add_task_label()
    self._add_video_id()
    self._add_frame_rate()

    print('is training = ', is_training, 'augment goals =', augment_goals,
          'sample triplets =', sample_triplets)
    regression_utils.sample_frames(
        ...
        stride=stride,
        sample_triplets=sample_triplets,
...
class FullVideoDatasetFactory(Ssv2DatasetFactory):
  ...
  def _build(self,
    ...
    regression_utils.load_full_video(
        parser_builder=self.parser_builder,
        sampler_builder=self.sampler_builder,
        decoder_builder=self.decoder_builder,
        preprocessor_builder=self.preprocessor_builder,
...
def load_split(
    ...
    ds_factory_kwargs: Dict[str, Any] = {}) -> Tuple[tf.data.Dataset, int]:
...
def get_dataset(
  ...
  sample_triplets = dataset_configs.get('sample_triplets', False)
  ...
  # Load one full sequence.
  test_batch_size = dataset_configs.get('num_test_clips')
  ...
  test_ds_factory = functools.partial(
      FullVideoDatasetFactory,
      base_dir=dataset_configs.base_dir,
      tables=dataset_configs.tables,
      examples_per_subset=dataset_configs.examples_per_subset,
      num_classes=dataset_configs.num_classes,
  ...
  def create_dataset_iterator(
    ...
    factory = test_ds_factory if is_test else ds_factory
    regression_kwargs = {
        'num_frames': num_frames,
        'sample_triplets': sample_triplets,
        'augment_goals': augment_goals and is_training}
    kwargs = {} if is_test else regression_kwargs
    ...
    if is_test:
      print('Testing with', num_spatial_crops, 'spatial crops')
  ...
  meta_data = {
      ...
      'mean_frame_rate': 11.841645719966037,
  }
  logging.info(
