...
def sum_metric_normalizer(metrics: Tuple[jnp.ndarray, jnp.ndarray]
                          ) -> Tuple[jnp.ndarray, jnp.ndarray]:
  """Applies sum over the given tuple of (metric, normalizer)."""
  summed_metric = jnp.sum(metrics[0])
  summed_normalizer = jnp.sum(metrics[1])
  return (summed_metric, summed_normalizer)


def num_frames(logits: jnp.ndarray,
...
def num_frames_div_two(logits: jnp.ndarray,
                       one_hot_targets: jnp.ndarray,
                       weights: Optional[jnp.ndarray] = None,
                       ) -> Union[jnp.ndarray, int]:
  del logits
  if weights is None:
    return one_hot_targets.shape[0] / 2
  return weights.sum() / 2


def num_frames_sub_one(logits: jnp.ndarray,
                       one_hot_targets: jnp.ndarray,
                       weights: Optional[jnp.ndarray] = None,
                       ) -> Union[jnp.ndarray, int]:
  return num_frames(logits, one_hot_targets, weights) - 1


def get_ranking(array: jnp.ndarray):
  array = jnp.squeeze(array, axis=1)
  argsort = jnp.argsort(array)
  ranking = jnp.empty_like(array)
  ranking = ranking.at[argsort].set(jnp.arange(len(array)))
  return ranking


def pmapped_spearman_correlation(preds, targets, weights):
  """Compute spearman correlation for a single video."""
  del weights
  ps = preds
  ts = targets
  ts = len(ts) - ts
  target_ranking = get_ranking(ts)
  pred_ranking = get_ranking(ps)
  r = jnp.corrcoef(target_ranking, pred_ranking)[0, 1]
  return jnp.array(r)


def spearman_correlation(preds, targets, weights):
  rs = []
  for ps, ts, ws in zip(preds, targets, weights):
    ps = ps[ws[1:].astype(bool)]
    ts = ts[ws[1:].astype(bool)]
    ts = len(ts) - ts
    target_ranking = get_ranking(ts)
    pred_ranking = get_ranking(ps)
    r = jnp.corrcoef(target_ranking, pred_ranking)[0, 1]
...
def pairwise_hinge_loss(preds, targets, weights):
  # Input is a batch of pairs (with predictions already accounting for goal).
  # If predictions are correctly ordered, each difference should be positive.
  batch_size = int(preds.shape[0] / 2)
  diffs = jnp.multiply(preds[:batch_size] - preds[batch_size:],
                       jnp.expand_dims(weights[:batch_size], axis=1))
  diffs = jnp.minimum(diffs, 0)
  loss = jnp.sum(jnp.abs(diffs))
  return loss


def hinge_loss(preds, targets, weights):
  # preds = preds[jnp.squeeze(targets)]
  # Input is one sequence (with the last item in the original sequence as goal).
  preds = jnp.squeeze(preds, axis=1)
  # If predictions are correctly ordered, each difference should be positive.
  diffs = jnp.multiply(preds[:-1] - preds[1:], weights[1:])
  diffs = jnp.minimum(diffs, 0)
  loss = jnp.sum(jnp.abs(diffs))
  return loss


def binary_misclassified_count(preds, targets, weights):
  preds = jnp.squeeze(preds, axis=1)
  diffs = jnp.multiply(preds[:-1] - preds[1:], weights[1:])
  misclassified_count = jnp.sum(diffs < 0)
