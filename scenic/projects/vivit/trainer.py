...
def train(
  ...
  best_eval_metrics = collections.defaultdict(list)
      ...
      with report_progress.timed('eval'):
        ...
        if config.checkpoint:
          for k, v in eval_summary.items():
            print(f'Step = {step}; {k} = {v:.12f} vs. {best_eval_metrics[k]}')
            if (len(best_eval_metrics[k]) < n_ckpts_to_keep
                or (k in loss_metrics and best_eval_metrics[k][-1] > v)
                or (k not in loss_metrics and best_eval_metrics[k][-1] > v)):
              if k in loss_metrics:
                if best_eval_metrics[k]:
                    print(f'  {v:.12f} < {best_eval_metrics[k][-1]}, saving ckpt')
                else:
                    print(f'  {v:.12f} < {best_eval_metrics[k]}, saving ckpt')
                # Keep the smallest value checkpoints.
                best_eval_metrics[k] = sorted(
                     best_eval_metrics[k][:n_ckpts_to_keep - 1] + [v])
                # Use -v in filename so flax loads min loss ckpt by default.
                v = -v
                print(f'New best_eval_metrics[k]: {best_eval_metrics[k]}')
              else:
                if best_eval_metrics[k]:
                  print(f'  {v:.12f} > {best_eval_metrics[k][-1]}, saving ckpt')
                else:
                ...
                best_eval_metrics[k] = sorted(
                    best_eval_metrics[k][:n_ckpts_to_keep - 1] + [v], reverse=True)
                print(f'New best_eval_metrics[k]: {best_eval_metrics[k]}')

              # TODO(minttu): Optimize by syncing only once (and only if at
              # least 1 ckpt should be saved)
