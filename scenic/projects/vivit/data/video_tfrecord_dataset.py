...
def get_video_length(sequence: Dict[str, tf.Tensor],
                     img_feature_name: str,
                     output_feature_name: str = 'video_length',
                     ) -> Dict[str, tf.Tensor]:
  seq_len = tf.shape(input=sequence[img_feature_name])[0]
  sequence[output_feature_name] = seq_len
  return sequence
...
class TFRecordDatasetFactory(video_dataset.BaseVideoDatasetFactory):
  ...
  def _add_video_length(
      self,
      input_feature_name: str = builders.IMAGE_FEATURE_NAME):
    self.sampler_builder.add_fn(
      lambda x: get_video_length(x, input_feature_name),
      fn_name="add_video_length")
